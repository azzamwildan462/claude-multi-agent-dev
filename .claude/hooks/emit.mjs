...
(async () => {
  ...
  //   tool_input, tool_response. For subagent tool calls Claude Code adds
  //   agent_id + agent_type at the top level — that's how we distinguish
  //   a subagent from the lead.
  const event = {
    type: TYPE,
...
// Claude Code adds `agent_type` (and `agent_id`) to hook payloads for every
// tool call made by a subagent — works for both built-in agents and
// `isolation: "worktree"` subagents. Lead-session calls omit these fields.
